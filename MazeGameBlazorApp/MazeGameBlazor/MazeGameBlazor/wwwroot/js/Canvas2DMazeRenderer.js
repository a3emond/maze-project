...
function renderMaze() {
    ...
    let startX = Math.floor(window.cameraX / tileSize);
    let startY = Math.floor(window.cameraY / tileSize);
    let endX = Math.min(window.mazeWidth, Math.ceil((window.cameraX + window.canvas.width) / tileSize));
    let endY = Math.min(window.mazeHeight, Math.ceil((window.cameraY + window.canvas.height) / tileSize));
    ...
    for (let y = startY; y < endY; y++) {
        for (let x = startX; x < endX; x++) {
            ...
            let drawX = (x * tileSize) - window.cameraX;
            let drawY = (y * tileSize) - window.cameraY;
...
function initMinimap(tileData, width, height) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            let tileType = tileData[tileIndex];
...
window.renderMinimap = function () {
    ...
    for (let y = 0; y < window.mazeHeight; y++) {
        for (let x = 0; x < window.mazeWidth; x++) {
            let tileIndex = y * window.mazeWidth + x;
            let tileType = window.tileData[tileIndex];
